...
//    paragraph.lineBreakMode = NSLineBreakByWordWrapping;
    //    paragraph.maximumLineHeight = 50;
...
#pragma mark 一串字符在固定宽度下，固定行数下,正常显示所需要的高度 method
+ (CGFloat)numberofline:(NSInteger)numberofline
   autoHeightWithString:(NSString *)string
                  Width:(CGFloat)width
                   Font:(UIFont *)font{
    
    NSMutableParagraphStyle *paragra = [[NSMutableParagraphStyle alloc] init];
    paragra.lineBreakMode = NSLineBreakByWordWrapping;
    NSDictionary *dic = @{
                          NSFontAttributeName:font,
                          NSParagraphStyleAttributeName:paragra,
                          };
    
   CGFloat strhight = [string boundingRectWithSize:CGSizeMake(width, CGFLOAT_MAX) options:NSStringDrawingUsesLineFragmentOrigin|NSStringDrawingUsesFontLeading attributes:dic context:nil].size.height;//文字多行显示高度
    ...
    CGFloat lineHeigh = [str boundingRectWithSize:CGSizeMake(width, CGFLOAT_MAX) options:NSStringDrawingUsesFontLeading|NSStringDrawingUsesLineFragmentOrigin attributes:dic context:nil].size.height;
    
    
    
    return ceilf(strhight)  >  ceilf(lineHeigh) ?  ceilf(lineHeigh) :  ceilf(strhight) ;
}
...
+ (CGFloat)autoHeightWithString:(NSString *)string
                          Width:(CGFloat)width
                           Font:(UIFont *)font
                         paragh:(NSParagraphStyle *)paragh{
    
    NSDictionary *dic = @{
                          NSFontAttributeName:font,
                          NSParagraphStyleAttributeName:paragh
                          };
